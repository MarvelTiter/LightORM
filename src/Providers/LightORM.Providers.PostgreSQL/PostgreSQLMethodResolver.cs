...
public sealed class PostgreSQLMethodResolver : BaseSqlMethodResolver
{
    ...
    {
        // PostgreSQL SUBSTRING(column FROM start FOR length)
        resolver.Sql.Append("SUBSTRING(");
        resolver.Visit(methodCall.Object);
        resolver.Sql.Append(" FROM (");
        resolver.Visit(methodCall.Arguments[0]);
        // (start + 1)
