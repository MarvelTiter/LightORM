...
public class SqlServerTableReader(ISqlExecutor ado) : Implements.ReadTypeFromTable
{
    public override async Task<IList<string>> GetTablesAsync()
    {
        string sql = "SELECT NAME TableName FROM SYSOBJECTS WHERE XTYPE = 'U' ORDER BY NAME";
        return await ado.QueryListAsync<string>(sql);
    }

    protected override Task<IList<ReadedTableColumn>> GetTableColumnsAsync(string table)
    {
        string sql = $@"
SELECT 
...
";
        return ado.QueryListAsync<ReadedTableColumn>(sql);
    }
    ...
    /// <returns></returns>
    protected override bool ParseDataType(ReadedTableColumn column, out string type)
    {
