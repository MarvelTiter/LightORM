...
internal static class CustomDatabaseExtensions
{
    ...
    extension(StringBuilder sql)
    {
        ...
        public StringBuilder AppendSimpleColumnParameter(SimpleColumn col, IDatabaseAdapter database)
        {
            //sql.WithPrefix(col.PropName, database);
            //sql.Append('_');
            //sql.Append(col.RowIndex);
            //if (col.IsNewVersion)
            //{
            //    sql.Append("_n");
            //}
            sql.WithPrefix(col.ValueName, database);
            return sql;
        }

        internal void AppendSimpleColumnValueExpression(SimpleColumn col, IDatabaseAdapter database)
        {
            ...
            AppendSimpleColumnParameter(sql, col, database);
        }

        private static string FormatStaticValue(IDatabaseAdapter database, object value)
        {
            return value switch
            {
        ...
        public StringBuilder AppendEntryColumns(ICollection<MapEntry> entries, string? prefix = null)
        {
        ...
        public StringBuilder AppendEntryColumns(ITableColumnInfo[] entries, IDatabaseAdapter database, string? prefix = null)
        {
            foreach (var entry in entries)
            {
                sql.Append(prefix);
                sql.AppendEmphasis(entry.ColumnName, database);
                sql.Append(',');
            }
            sql.RemoveLast(1);
            return sql;
