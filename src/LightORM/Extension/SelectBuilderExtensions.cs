...
internal static class BuilderExtensions
{
    ...
    {
        public void AddUnion(SelectBuilder select, bool all)
        {
            select.IsUnion = true;
        ...
        public void HandleTempsRecursion(SelectBuilder temp)
        {
            foreach (var item in temp.TempViews)
            {
                builder.HandleTempsRecursion(item);
            }
            temp.TempViews.Clear();
            if (builder.TempViews.Any(s => s.Id == temp.Id))
            {
                return;
            }
            if (temp.IsTemp)
            {
                builder.TempViews.Add(temp);
            }
        }
    }
}
