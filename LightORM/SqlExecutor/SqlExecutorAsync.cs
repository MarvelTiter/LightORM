...
namespace MDbContext.SqlExecutor
{
    ...
    public static partial class SqlExecutor
    {
        ...
        private static async Task<object> InternalExecuteScalarAsync(IDbConnection conn, CommandDefinition command, CancellationToken token)
        {
            // 缓存
            var parameter = command.Parameters;
        ...
        private static async Task<int> InternalExecuteNonQueryAsync(IDbConnection conn, CommandDefinition command, CancellationToken token)
        {
            // 缓存
            var parameter = command.Parameters;
            Certificate certificate = new Certificate(command.CommandText, command.CommandType, conn, typeof(object), parameter?.GetType());
